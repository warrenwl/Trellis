...
cat .trellis/spec/unit-test/index.md # 测试指南
```
...
| **简单任务** | 目标清晰，1-2 个文件，范围明确 | 快速确认 → 实现 |
| **复杂任务** | 目标模糊、多个文件、架构决策 | **Brainstorm → 任务工作流** |
...
> **如果有疑问，使用 Brainstorm + 任务工作流。**
>
> 任务工作流确保代码规范上下文被注入到代理，产生更高质量的代码。
> 开销很小，但收益显著。
...
2. 如果是，进入**任务工作流第 1 阶段路径 B**（创建任务、编写 PRD，然后研究）
3. 如果不是，澄清并再次确认
...
对于复杂或模糊的任务，使用 brainstorm 流程来澄清需求。
...
> **子任务分解**：如果 brainstorm 揭示多个独立工作项，考虑使用 `--parent` 标志或 `add-subtask` 命令创建子任务。
> 详见 `/trellis:brainstorm` 第 8 步。
...
#### 路径 A：来自 Brainstorm（跳到第 2 阶段）

PRD 和任务目录已在 brainstorm 中存在。直接跳到第 2 阶段。
...
继续之前必须有：
- [ ] 目标代码规范文件已确定
- [ ] 明确定义了具体契约（签名、字段、环境密钥）
- [ ] 明确定义了验证和错误矩阵
- [ ] 至少定义了一个 Good/Base/Bad 情况
...
| `python3 ./.trellis/scripts/task.py add-context` | 将代码规范/上下文文件添加到 jsonl |
