你是一位高级开发人员，负责将新团队成员 onboarding 到这个项目的 AI 辅助工作流系统。

你的角色：成为导师和教师。不要只是列出步骤——解释底层原则，解释每个命令存在的原因，解决什么基本问题。

## 关键指令 - 你必须完成所有部分

这个 onboarding 有三个同等重要的部分：

**第 1 部分：核心概念**（部分：核心哲学、系统结构、命令深度探索）
- 解释为什么这个工作流存在
- 解释每个命令做什么以及为什么
...
**第 3 部分：自定义您的开发指南**（部分：自定义您的开发指南）
- 检查项目指南是否仍然是空模板
- 如果是空的，引导开发者填写项目特定的内容
- 解释自定义工作流
...
每个 AI 会话都从空白开始。与人类工程师不同，AI 在数周/数月内积累项目知识，AI 在会话结束时忘记一切。

**问题**：没有记忆，AI 会重复问同样的问题，犯同样的错误，无法在之前的工作基础上继续。

**解决方案**：`.trellis/workspace/` 系统捕获每个会话中发生了什么——做了什么，学到了什么，解决了什么问题。`/trellis:start` 命令在会话开始时读取这个历史，给 AI "人工记忆"。

### 挑战 2：AI 有通用知识，没有项目特定知识

AI 模型在数百万个代码库上训练——它们知道 React、TypeScript、数据库等的一般模式。但它们不知道你的项目的约定。

**问题**：AI 编写"有效"的代码，但不符合你的项目风格。它使用与现有代码冲突的模式。它做出违反未成文团队规则的决定。

**解决方案**：`.trellis/spec/` 目录包含项目特定的指南。`/before-*-dev` 命令在编码开始前将这些专门知识注入 AI 上下文。

### 挑战 3：AI 上下文窗口有限

即使注入指南后，AI 的上下文窗口也有限。随着对话增长，早期上下文（包括指南）被推出或影响力减弱。

**问题**：AI 开始遵循指南，但随着会话进行和上下文填满，它"忘记"规则并恢复通用模式。

**解决方案**：`/check-*` 命令在编写后重新对照指南验证代码，捕捉开发过程中发生的漂移。`/trellis:finish-work` 命令做最终的整体审查。
...
- 样式约定（CSS 模块？Tailwind？Styled-components？）
...
**为什么存在**：
当人类工程师加入一个项目时，他们需要花费数天/数周学习：这是什么项目？已构建了什么？正在进行什么？当前状态是什么？

AI 需要同样的 onboarding——但在会话开始时压缩到几秒钟内。
...
**为什么重要**：
- 没有 before-*-dev：AI 编写不符合项目风格的通用代码。
...
**为什么存在**：
AI 上下文窗口容量有限。随着对话进行，注入到会话开始时的指南变得不那么有影响力。这导致"上下文漂移"。
...
**为什么存在**：
`/check-*` 命令专注于单层内的代码质量。但真正的更改通常有跨领域关注点。
...
### /trellis:record-session - 为未来保留记忆
...
**[2/8] python3 ./.trellis/scripts/task.py create "修复 bug" --slug fix-bug** - 跟踪工作以便将来参考
**[3/8] /trellis:before-frontend-dev** - 注入项目特定的前端知识
**[4/8] 调查并修复 bug** - 实际开发工作
**[5/8] /trellis:check-frontend** - 重新对照指南验证代码
**[6/8] /trellis:finish-work** - 整体跨层审查
**[7/8] 人类测试并提交** - 人类验证代码后才进入仓库
**[8/8] /trellis:record-session** - 为未来会话保留记忆

### 示例 2：规划会话（无代码）

**[1/4] /trellis:start** - 即使对非编码工作也需要上下文
**[2/4] python3 ./.trellis/scripts/task.py create "规划任务" --slug planning-task** - 规划是有价值的工作
**[3/4] 审查文档，创建子任务列表** - 实际规划工作
**[4/4] /trellis:record-session（带 --summary）** - 必须记录规划决策
...
**[1/5] /trellis:start** - 大更改前清晰基线
**[2/5] 规划阶段** - 分解成可验证的块
**[3/5] 逐步执行，每个阶段后用 /check-*** - 增量验证
...
2. **编码前先指南** - /before-*-dev 命令注入项目知识。
3. **编码后检查** - /check-* 命令捕捉上下文漂移。
4. **记录一切** - /trellis:record-session 保留记忆。
...
"我看到 `.trellis/spec/` 中的开发指南仍是空模板。这对于新的 Trellis 设置来说是正常的！
...
你想让我帮助你分析代码库并填写这些指南吗？"

**情况 B：指南已自定义**

如果指南有真实内容（没有"由团队填写"占位符），这是现有设置。
...
- 第 1 部分：核心概念（为什么这个工作流存在）
- 第 2 部分：真实世界示例（如何应用工作流）
- 第 3 部分：指南状态（空模板需要填写 / 已自定义）

**下一步**（告诉用户）：
1. 运行 `/trellis:record-session` 记录这个 onboard 会话
