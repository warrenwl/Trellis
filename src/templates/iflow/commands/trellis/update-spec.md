...
当您从调试、实施或讨论中学到有价值的内容时，使用此命令更新相关代码规范文档。
...
| **新思考触发** | "在做 Y 之前别忘了检查 X" | `guides/*.md`（作为检查清单项，而非详细规则）|

**关键洞察**：代码规范更新不仅仅针对问题。每个功能实施都包含未来 AI/开发者需要安全执行的设计决策和契约。
...
| 类型 | 位置 | 目的 | 内容风格 |
|------|----------|---------|---------------|
| **代码规范** | `backend/*.md`、`frontend/*.md` | 告诉 AI"如何安全实施" | 签名、契约、矩阵、情况、测试点 |
| **指南** | `guides/*.md` | 帮助 AI"考虑什么" | 检查清单、问题、指向规范的指针 |

**决策规则**：问自己：

- 这是**如何编写**代码 → 放入 `backend/` 或 `frontend/`
- 这是**写之前**考虑什么 → 放入 `guides/`
...
- `/trellis:break-loop` - 深度分析 bug，经常揭示需要的规范更新
...
> **代码规范是活文档。每次调试会话、每次"顿悟"都是让实施契约更清晰的机会。**
