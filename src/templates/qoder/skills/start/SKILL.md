...
| **简单修复** | 拼写错误、注释更新、单行更改，< 5 分钟 | 直接编辑 |
| **开发任务** | 任何代码更改：修改逻辑、添加功能、修复 bug、触达多个文件 | **任务工作流** |

### 决策规则

> **如果有疑问，使用任务工作流。**
>
> 任务工作流确保规范被注入到 agent，产生更高质量的代码。
> 开销很小，但收益显著。

> **子任务分解**：如果任务有多个独立工作项，
> 考虑使用 `--parent` 标志或 `add-subtask` 命令创建子任务。
> 详见 brainstorm skill 的步骤 8。
...
根据确认的 PRD，调用研究 Agent 找到相关规范和模式：
