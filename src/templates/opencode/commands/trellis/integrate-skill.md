...
将 Claude 全局 skill 适配并集成到项目的开发指南中（不是直接集成到项目代码）。
...
> - 代码示例 → 放入 `.trellis/spec/{target}/examples/skills/<skill-name>/`
